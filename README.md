...
### 🏋️ Proje 11 — [sporkulubu](./11-spor-kulubu-dapper-projesi/)
...
ASP.NET Core ile Dapper ORM kullanılarak Stored Procedure ve ADO.NET teknolojisiyle geliştirilmiş **Spor Kulübü & Antrenman Yönetim Sistemi**. Kullanıcı giriş/kayıt, üyelerin aranması, üye listelerinin Türkçe karakter uyumlu PDF ve Excel çıktılarının alınması, antrenörlerin seans bazlı hasılat ve verim raporlamaları yürütülmektedir.

**Öne Çıkan Özellikler:**
- **Çoklu Başlangıç Projesi:** Solution çalıştırıldığında API ve MVC katmanları aynı anda ayağa kalkar.
- **Identity ve JWT Entegrasyonu:** API tarafında token doğrulaması, MVC tarafında ise oturum yönetimi için JWT içeren Cookie Authentication.
- **Yönetici Giriş ve Güvenlik:** Varsayılan admin seeding yapısı ve üye kayıt formu rolden bağımsızlaştırılmıştır.
- **Görsel Arayüz (Premium Landing Page):** Giriş yapmamış ziyaretçilere özel, menüsüz, tam sayfa şık bir tanıtım sayfası (Landing Page). Giriş yapıldığında ise tam yetkili Sidebar Dashboard yapısı.
- **Dinamik Raporlama (10 Rapor):** Dropdown ile seçilebilen 10 farklı spor analiz sorgusu.
- **Rapor Export:** Üye Listesinden PDF (iTextSharp) ve Excel (EPPlus) çıktısı alma.
- **Memory Cache & ILogger:** Branş verileri için Memory Cache entegrasyonu ve erişim loglamaları.
